    ...
    def _get_target_specific_post_transformers(
        ...
        if transform_mode.from_damage() and transform_mode.to_buff():
            ...
            for key, value in JavaScriptTarget.TRANSLATION_PAIRS.items():
                result.append(SingleWordReplacer(keyword=key, replacement=f"{value}.value"))
            result.append(
                # IMPORTANT:
                ...
                RegexReplacer(
                    ...
                    replacement_provider=(
                        lambda context:
                            f'({JavaScriptTarget.BASE_ATTACK_MODIFIER_FUNCTION}.value)('
...
class FormulaContext:
    ...
    def replace(self, text: str, start: int, end: int, allow_shorten: bool) -> None:
        ...
        self._shift(start + 1, new_length - old_length, allow_shorten)
...
class DamageBuffInjector(BuffInjector):
    ...
    def _inject_pre_max_3_bracket(self, context: MatchContext) -> None:
        context.replace("MAX(", subpattern_name="attack_bracket_1")
        context.replace("MAX(", subpattern_name="attack_bracket_2")
        context.replace("MAX(", subpattern_name="attack_bracket_3")

    def _inject_pre_max_1_bracket(self, context: MatchContext) -> None:
        context.replace("MAX(", subpattern_name="attack_bracket")

    def _inject_attack_first_value(self, context: MatchContext) -> None:
        context.insert("+BuffDamageAttackFirstValue,0", after="attack_first_value")
    ...
    def _inject_attack_first_ratio(self, context: MatchContext) -> None:
        ...
        if context.match.group("attack_first_ratio") is not None:
            context.replace("MAX(", subpattern_name="attack_first_ratio_bracket")
            self._inject_annotated_buffs(
                AnnotatedBuff("@MonoEnergizedAttack", mono_buff),
                subpattern_name="attack_first_ratio",
                context=context,
            )
            context.insert(f'{buff},0', after="attack_first_ratio")
        else:
            context.insert(
                f'*MAX(1+(N("@MonoEnergizedAttack")+MAX({mono_buff})){buff},0)',
    ...
    def _inject_attack_final_value(self, context: MatchContext) -> None:
        ...
        context.insert(f'{buff},0', after="attack_final_value")
    ...
    def _get_attack_gain_pass(
        ...
        buff = buffs[damage_type]
        return lambda context: context.insert(f'{buff},0', after="attack_gain")

    def _get_damage_weakener_pass(
        self, damage_type: Literal["physical", "magical"]
    ) -> Callable[[MatchContext], None]:
        if damage_type == "physical":
            ...
            inner_limit_exist = False
            inner_limit_group = None
            inner_limit_prefix = ""
            inner_limit_suffix = ""
            ...
            inner_limit_exist = True
            inner_limit_group = "resist_bracket"
            inner_limit_prefix = "MIN("
            inner_limit_suffix = ",100)"
        ...
        def inject_weakener(context: MatchContext) -> None:
            if context.match.group(weakener_ungrouped) is None:
                if inner_limit_exist:
                    context.insert(inner_limit_prefix, before=inner_limit_group)
                ...
                if inner_limit_exist:
                    context.insert(inner_limit_suffix, after=weakener_loss_ratio)
                ...
                context.insert(f'MAX(({inner_limit_prefix}(', before=weakener_ungrouped)
                ...
                context.insert(
                    f'{loss_value_buff})){loss_ratio_buff}{inner_limit_suffix}'
    ...
    def _get_damage_final_ratio_pass(
        ...
        def _inject_damage_final_ratio(context: MatchContext) -> None:
            ...
            if context.match.group("damage_final_ratio") is not None:
                if 'N("@NoBuff")' not in context.get("damage_final_ratio"):
                    self._inject_annotated_buffs(
                        (
...
class PhysicalDamageBuffInjector(DamageBuffInjector):
    ...
    def _get_passes(self) -> Iterable[Callable[[MatchContext], None]]:
        return [
            self._inject_pre_max_3_bracket,
    ...
    def _ensure_valid(self, match: re.Match) -> None:
        ...
        Assert.has_key(groups, [
            "physical_line_1", "attack_bracket_1", "attack_bracket_2", "attack_bracket_3",
            "id", "attack_first_value", "attack_first_ratio_bracket", "attack_first_ratio",
        ...
        Assert.not_empty(groups, [
            "physical_line_1", "physical_line_2", "id",
            "attack_bracket_1", "attack_bracket_2", "attack_bracket_3",
        ])
        Assert.equal(groups, "physical_line_1", "physical_line_2")
        Assert.true(
            (
                Check.not_empty(groups, "attack_first_ratio_bracket") and
                Check.not_empty(groups, "attack_first_ratio")
            ) or (
                Check.not_none(groups, "attack_first_ratio_empty")
...
class MagicalDamageBuffInjector(DamageBuffInjector):
    ...
    def _ensure_valid(self, match: re.Match) -> None:
        groups = match.groupdict()
        Assert.has_key(groups, [
            "magical_main", "attack_bracket_1", "attack_bracket_2", "attack_bracket_3",
            "id", "attack_first_value", "attack_first_ratio_bracket", "attack_first_ratio",
            "attack_first_ratio_empty", "attack_final_value", "attack_multiplier",
            "attack_gain", "resist_bracket", "resist_loss_value", "resist_loss_value_ungrouped",
        ...
        Assert.true(
            (
                Check.not_empty(groups, "resist_bracket") and
...
class TrueDamageBuffInjector(DamageBuffInjector):
    ...
    def _ensure_valid(self, match: re.Match) -> None:
        groups = match.groupdict()
        Assert.has_key(groups, [
            "true_main", "attack_bracket_1", "attack_bracket_2", "attack_bracket_3",
            "id", "attack_first_value", "attack_first_ratio_bracket", "attack_first_ratio",
            "attack_first_ratio_empty", "attack_final_value", "attack_multiplier",
...
class ElementalDamageBuffInjector(DamageBuffInjector):
    ...
    def _ensure_valid(self, match: re.Match) -> None:
        groups = match.groupdict()
        Assert.has_key(groups, [
            "elemental_main", "attack_bracket_1", "attack_bracket_2", "attack_bracket_3",
            "id", "attack_first_value", "attack_first_ratio_bracket", "attack_first_ratio",
            "attack_first_ratio_empty", "attack_final_value", "attack_multiplier",
            "attack_gain", "damage_final_ratio", "damage_final_ratio_empty",
        ])
...
class DefaultElementalDamageBuffInjector(DamageBuffInjector):
    ...
    def _get_passes(self) -> Iterable[Callable[[MatchContext], None]]:
        return [
            self._inject_pre_max_1_bracket,
            self._get_attack_gain_pass(damage_type="elemental"),
            self._get_damage_final_ratio_pass(damage_type="elemental"),
        ]
...
class DirectInjuryBuffInjector(InjuryBuffInjector):
    ...
    def _ensure_valid(self, match: re.Match) -> None:
        groups = match.groupdict()
        Assert.has_key(groups, [
            "injury_main", "attack_bracket_1", "attack_bracket_2", "attack_bracket_3",
            "injury_type", "id", "attack_first_value",
            "attack_first_ratio_bracket", "attack_first_ratio", "attack_first_ratio_empty",
...
class PhysicalInjuryBuffInjector(InjuryBuffInjector):
    ...
    def _ensure_valid(self, match: re.Match) -> None:
        ...
        Assert.has_key(groups, [
            "physical_line_1", "attack_bracket_1", "attack_bracket_2", "attack_bracket_3",
            "injury_type", "id", "attack_first_value", 
            "attack_first_ratio_bracket", "attack_first_ratio",
            "attack_first_ratio_empty", "attack_final_value", "attack_multiplier",
            "attack_gain", "physical_line_2", "defense_loss_value",
            "defense_loss_value_ungrouped", "defense_loss_value_empty",
            "defense_loss_ratio", "defense_ignore_value",
...
class MagicalInjuryBuffInjector(InjuryBuffInjector):
    ...
    def _ensure_valid(self, match: re.Match) -> None:
        ...
        Assert.has_key(groups, [
            "injury_main", "attack_bracket_1", "attack_bracket_2", "attack_bracket_3",
            "injury_type", "id", "attack_first_value", "attack_first_ratio_bracket",
            "attack_first_ratio", "attack_first_ratio_empty", "attack_final_value",
            "attack_multiplier", "attack_gain", "resist_bracket", "resist_loss_value",
    ...
    def _minify_arithmetics(self, formula: str) -> str:
        ...
        while True:
            ...
            for arithmetic, answer in answer_map.items():
                formula = formula.replace(arithmetic, answer)
        return formula
...
class JavaScriptTarget:
    BASE_ATTACK_MODIFIER_FUNCTION = "controlFuncModifyBaseAttack"
    TRANSLATION_PAIRS = {
        "EnemyDefenseMajor": "controlEnemyMajorDefense",
        "EnemyResistanceMajor": "controlEnemyMajorResistance",
        "EnemyElementalResistanceMajor": "controlEnemyMajorElementalResistance",
        "EnemyInjuryResistanceMajor": "controlEnemyMajorInjuryResistance",
        "EnemyRankMajor": "controlEnemyMajorRank",
        "EnemyMaxElementMajor": "controlEnemyMajorMaxElement",
        "EnemyWeightMajor": "controlEnemyMajorWeight",
        "EnemyCountMinor": "controlEnemyMinorCount",
        "EnemyDefenseMinor": "controlEnemyMinorDefense",
        "EnemyResistanceMinor": "controlEnemyMinorResistance",
        "EnemyElementalResistanceMinor": "controlEnemyMinorElementalResistance",
        "EnemyInjuryResistanceMinor": "controlEnemyMinorInjuryResistance",
        "EnemyRankMinor": "controlEnemyMinorRank",
        "EnemyMaxElementMinor": "controlEnemyMinorMaxElement",
        "EnemyWeightMinor": "controlEnemyMinorWeight",
        "EnemyDamageTimeWindow": "controlEnemyDamageTimeWindow",
        "EnemyAerial": "controlEnemyAerial",
        "EnemySeparated": "controlEnemySeparated",
        "BuffSourceIds": "controlBuffSourceIds",
        "BuffDamageApplyToSingleAllyOnly": "controlBuffDamageApplyToSingleAllyOnly",
        "BuffDamageApplyToNonSummonAllyOnly": "controlBuffDamageApplyToNonSummonAllyOnly",
        "BuffDamageApplyToSingleEnemyOnly": "controlBuffDamageApplyToSingleEnemyOnly",
        "BuffDamageAttackFirstValue": "controlBuffDamageAttackFirstValue",
        "BuffDamageAttackFinalValue": "controlBuffDamageAttackFinalValue",
        "BuffDamageAttackFirstRatio": "controlBuffDamageAttackFirstRatio",
        "BuffDamageAttackFirstRatioVanguard": "controlBuffDamageAttackFirstRatioVanguard",
        "BuffDamageAttackFirstRatioGuard": "controlBuffDamageAttackFirstRatioGuard",
        "BuffDamageAttackFirstRatioDefender": "controlBuffDamageAttackFirstRatioDefender",
        "BuffDamageAttackFirstRatioSniper": "controlBuffDamageAttackFirstRatioSniper",
        "BuffDamageAttackFirstRatioCaster": "controlBuffDamageAttackFirstRatioCaster",
        "BuffDamageAttackFirstRatioMedic": "controlBuffDamageAttackFirstRatioMedic",
        ...
        "BuffDamageAttackSpeedFirstValueMelee": "controlBuffDamageAttackSpeedFirstValueMelee",
        "BuffDamageAttackSpeedFirstValueRanged": "controlBuffDamageAttackSpeedFirstValueRanged",
        "BuffDamagePhysicalGainValue": "controlBuffDamagePhysicalGainValue",
        "BuffDamageMagicalGainValue": "controlBuffDamageMagicalGainValue",
        "BuffDamageTrueGainValue": "controlBuffDamageTrueGainValue",
        "BuffDamageElementalGainValue": "controlBuffDamageElementalGainValue",
        "BuffDamagePhysicalFinalRatio": "controlBuffDamagePhysicalFinalRatio",
        "BuffDamageMagicalFinalRatio": "controlBuffDamageMagicalFinalRatio",
        "BuffDamageTrueFinalRatio": "controlBuffDamageTrueFinalRatio",
        ...
        "BuffDamageEnemyDefenseLossValue": "controlBuffDamageEnemyDefenseLossValue",
        "BuffDamageEnemyDefenseLossFinalRatio": "controlBuffDamageEnemyDefenseLossFinalRatio",
        "BuffDamageEnemyResistanceLossValue": "controlBuffDamageEnemyResistanceLossValue",
        "BuffDamageEnemyResistanceLossFinalRatio": "controlBuffDamageEnemyResistanceLossFinalRatio",
        "BuffDamageEnemyDefenseIgnoreValue": "controlBuffDamageEnemyDefenseIgnoreValue",
        "BuffDamageEnemyDefenseIgnoreRatio": "controlBuffDamageEnemyDefenseIgnoreRatio",
        "BuffDamageEnemyResistanceIgnoreValue": "controlBuffDamageEnemyResistanceIgnoreValue",
        "BuffDamageEnemyResistanceIgnoreRatio": "controlBuffDamageEnemyResistanceIgnoreRatio",
        "BuffDamageMonoEncouragedAttackFinalValue": "controlBuffDamageMonoEncouragedAttackFinalValue",
        "BuffDamageMonoEnergizedAttackFirstRatio": "controlBuffDamageMonoEnergizedAttackFirstValue",
        "BuffDamageMonoEnergizedAttackFirstRatioMelee": "controlBuffDamageMonoEnergizedAttackFirstValueMelee",
        "BuffDamageMonoEnemyStolenDefenseLossValue": "controlBuffDamageMonoEnemyStolenDefenseLossValue",
        "BuffDamageMonoEnemyFrozenResistanceLossValue": "controlBuffDamageMonoEnemyFrozenResistanceLossValue",
        "BuffDamageMonoEnemyVulnerableFinalRatio": "controlBuffDamageMonoEnemyVulnerableFinalRatio",
        "BuffDamageMonoEnemyVulnerablePhysicalFinalRatio": "controlBuffDamageMonoEnemyVulnerablePhysicalFinalRatio",
        "BuffDamageMonoEnemyVulnerableMagicalFinalRatio": "controlBuffDamageMonoEnemyVulnerableMagicalFinalRatio",
        "BuffDamageMonoEnemyVulnerableElementalFinalRatio": "controlBuffDamageMonoEnemyVulnerableElementalFinalRatio",
        ...
        "BuffDamageSkillPointSupplyEnablesDefensive": "controlBuffDamageSkillPointSupplyEnablesDefensive",
    }
  ...
  (?P<physical_line_1>
    (?P<attack_bracket_1>\()\s*
      (?P<attack_bracket_2>\()\s*
        ...
        \s*\)
        (?:
          \*(?P<attack_first_ratio_bracket>\()\s*1(?P<attack_first_ratio>(?:
